...
def test_api_endpoint(method, endpoint, data=None, headers=None, expected_status=200):
    ...
    try:
        ...
        try:
            ...
            print(
                f"Resposta: {json.dumps(response_data, indent=2, ensure_ascii=False)}"
            )
        ...
        if response.status_code == expected_status:
            ...
            print(
                f"⚠️ Teste Falhou! Status esperado: {expected_status}, mas foi recebido: {response.status_code}"
            )
            return response
...
def main():
    ...
    register_client_response = test_api_endpoint(
        "POST", "/api/auth/register", client_data, expected_status=201
    )
    ...
    test_api_endpoint(
        "POST", "/api/auth/register", restaurant_data, expected_status=201
    )
    ...
    if login_response and login_response.status_code == 200:
        ...
        test_api_endpoint(
            "GET", "/api/auth/profile", headers=headers, expected_status=200
        )
    else:
