...
def process_all_payouts():
    ...
    if not all([supabase_url, supabase_key, mp_access_token]):
        print("ERRO: Verifique se SUPABASE_URL, SUPABASE_SERVICE_KEY e MERCADO_PAGO_ACCESS_TOKEN estão no .env")
        return
    ...
    try:
        ...
        ).eq('status_pagamento', 'approved').eq('payout_status', 'pending').execute()
    ...
    if restaurant_payouts:
        ...
        try:
            resp_profiles = supabase.table('restaurant_profiles').select('id, restaurant_name, pix_key, mp_account_id').in_('id', restaurant_ids).execute()
            for profile in resp_profiles.data:
                partner_id = profile['id']
                final_payout_list.append({
                    "partner_type": "restaurant", "partner_id": partner_id,
                    "partner_name": profile.get('restaurant_name', 'N/A'),
                    "amount": f"{restaurant_payouts[partner_id]:.2f}",
                    "payment_details": { "pix_key": profile.get('pix_key'), "mp_account_id": profile.get('mp_account_id') },
    ...
    for payout_job in final_payout_list:
        ...
        try:
            ...
            response = requests.post(payout_api_url, json=payout_data, headers=request_headers)
            response_json = response.json()
            print(f"  -> Resposta do Servidor: Status={response.status_code}, Corpo={response_json}")

            if response.status_code in [200, 201]:
                print(f"  -> Sucesso! Payout criado com ID: {response_json.get('id')}")
                try:
                    update_resp = supabase.table('orders').update({'payout_status': 'processed'}).in_('id', order_ids_to_update).execute()
                    print(f"  -> {len(update_resp.data)} pedidos atualizados para 'processed' no banco de dados.")
                except Exception as db_error:
                    print(f"  -> ALERTA GRAVE: Payout realizado, mas FALHOU ao atualizar o status no banco de dados: {db_error}")
            else:
