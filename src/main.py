...
try:
    ...
    from src.routes.admin_logs import (
        admin_logs_bp,
    )  # IMPORTANTE: novo blueprint de logs
    from src.routes.admin_users import (
        admin_users_bp,
    )  # Blueprint for admin users API endpoints
...
app.register_blueprint(
    admin_logs_bp
)  # BLUEPRINT DE LOGS, sem url_prefix para endpoint /api/logs
app.register_blueprint(
    admin_users_bp
)  # BLUEPRINT DE ADMIN USERS, sem url_prefix para endpoints /api/users
...
def cors_test():
    ...
    try:
        ...
        return (
            jsonify({"error": "Erro interno no /api/cors-test", "details": str(e)}),
            500,
        )
