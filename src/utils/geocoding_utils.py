...
def geocode_address(street, number, neighborhood, city, state, zipcode):
    ...
    headers = {"User-Agent": "InksaDeliveryApp/1.0 (contact@yourdomain.com)"}

    # Endpoint da API Nominatim
    nominatim_url = "https://nominatim.openstreetmap.org/search"
    params = {"q": full_address, "format": "json", "limit": 1, "addressdetails": 0}
    ...
    try:
        ...
        if results and len(results) > 0:
            ...
            print(
                f"Geocodificação bem-sucedida para '{full_address}': Lat={lat}, Lon={lon}"
            )
            return lat, lon
        else:
            print(
                f"Geocodificação falhou para o endereço: '{full_address}'. Nenhum resultado encontrado."
            )
            return None, None
        ...
        print(
            f"Erro ao processar resposta JSON da geocodificação para '{full_address}': {e}"
        )
        return None, None
...
if __name__ == "__main__":
    ...
    lat, lon = geocode_address(
        "Rua XV de Novembro", "1000", "Centro", "Lages", "SC", "88501-000"
    )
    if lat and lon:
