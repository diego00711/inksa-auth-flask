...
def is_valid_status_transition(current_status, new_status):
    ...
    valid_transitions = {
        ...
        "ready": [
            "delivering",
            "cancelled",
        ],  # A transição para 'delivering' será controlada pelo endpoint /pickup
        "delivering": [
            "delivered"
        ],  # A transição para 'delivered' será controlada pelo endpoint /complete
        "delivered": [],
        "cancelled": [],
    }
...
def handle_orders():
    ...
    try:
        user_auth_id, user_type, error = get_user_id_from_token(
        ...
        if request.method == "GET":
            ...
            if sort_by not in valid_sort_columns:
                return (
                    jsonify(
                        {
                            "error": f"Campo de ordenação inválido. Use: {', '.join(valid_sort_columns)}"
                        }
                    ),
                    400,
                )
            ...
            if user_type == "restaurant":
                with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                    cur.execute(
                        "SELECT id FROM restaurant_profiles WHERE user_id = %s",
                        (user_auth_id,),
                    )
                    profile = cur.fetchone()
                    if not profile:
                        return (
                            jsonify({"error": "Perfil do restaurante não encontrado"}),
                            404,
                        )
            ...
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                for row in cur.fetchall():
                    order_dict = dict(row)
                    order_dict["status_display"] = STATUS_DISPLAY.get(
                        order_dict.get("status"), "Desconhecido"
                    )
        ...
        # --- POST: Criar novo pedido ---
            ...
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    "SELECT id FROM client_profiles WHERE user_id = %s", (user_auth_id,)
                )
                ...
                order_dict["status_display"] = STATUS_DISPLAY.get(
                    order_dict["status"], order_dict["status"]
                )
...
def update_order_status(order_id):
    ...
    try:
        ...
        if user_type != "restaurant":
            return (
                jsonify(
                    {"error": "Apenas restaurantes podem alterar o status de um pedido"}
                ),
                403,
            )
        ...
        if new_status_display not in VALID_STATUSES:
            return (
                jsonify(
                    {
                        "error": f"Status inválido. Válidos: {list(VALID_STATUSES.keys())}"
                    }
                ),
                400,
            )
        ...
        if new_status_internal in ["delivering", "delivered"]:
            return (
                jsonify(
                    {
                        "error": f"Para mudar o status para '{new_status_display}', use o endpoint de verificação de código apropriado."
                    }
                ),
                400,
            )
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute(
                "SELECT o.status FROM orders o JOIN restaurant_profiles rp ON o.restaurant_id = rp.id WHERE o.id = %s AND rp.user_id = %s",
            ...
            if not order:
                return (
                    jsonify(
                        {
                            "error": "Pedido não encontrado ou não pertence a este restaurante"
                        }
                    ),
                    404,
                )
...
def pickup_order(order_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute(
                "SELECT status, pickup_code FROM orders WHERE id = %s", (str(order_id),)
            )
            order = cur.fetchone()
            ...
            if order["status"] != "ready":
                return (
                    jsonify(
                        {
                            "error": f"Pedido não está pronto para retirada. Status atual: {STATUS_DISPLAY.get(order['status'])}"
                        }
                    ),
                    400,
                )
            ...
            cur.execute(
                "UPDATE orders SET status = 'delivering', updated_at = %s WHERE id = %s",
            ...
            return (
                jsonify(
                    {
                        "status": "success",
                        "message": "Pedido retirado e em rota de entrega.",
                    }
                ),
                200,
            )
...
def complete_order(order_id):
    ...
    try:
        ...
        if user_type not in ["restaurant", "driver"]:
            return (
                jsonify({"error": "Acesso não autorizado para completar a entrega"}),
                403,
            )
        ...
        if not data or "delivery_code" not in data:
            return (
                jsonify({"error": "Código de entrega (delivery_code) é obrigatório"}),
                400,
            )
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute(
                "SELECT status, delivery_code FROM orders WHERE id = %s",
                (str(order_id),),
            )
            order = cur.fetchone()
            ...
            if order["status"] != "delivering":
                return (
                    jsonify(
                        {
                            "error": f"O pedido não está em rota de entrega. Status atual: {STATUS_DISPLAY.get(order['status'])}"
                        }
                    ),
                    400,
                )
            ...
            cur.execute(
                "UPDATE orders SET status = 'delivered', updated_at = %s WHERE id = %s",
                (datetime.now(), str(order_id)),
            )
            conn.commit()
            ...
            return (
                jsonify(
                    {"status": "success", "message": "Pedido entregue com sucesso!"}
                ),
                200,
            )
...
def get_order_status_history(order_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            if user_type == "restaurant":
                cur.execute(
                    "SELECT o.* FROM orders o JOIN restaurant_profiles rp ON o.restaurant_id = rp.id WHERE o.id = %s AND rp.user_id = %s",
                    (str(order_id), user_auth_id),
                )
            elif user_type == "client":
                cur.execute(
                    "SELECT o.* FROM orders o JOIN client_profiles cp ON o.client_id = cp.id WHERE o.id = %s AND cp.user_id = %s",
                    (str(order_id), user_auth_id),
                )
            else:
            ...
            return (
                jsonify(
                    {"status": "success", "order_id": str(order_id), "history": history}
                ),
                200,
            )
