        ...
        if request.method == 'GET':
            ...
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                if (not _retirada_o) and _rest is not None and (_rest.get('delivery_type') or 'platform') == 'platform':
                    ...
                    if _online is not None and _online <= 0:
                        return jsonify({
...
@orders_bp.route('/<uuid:order_id>/atalho-de-volta', methods=['POST'])
@limiter.limit("20 per minute")
def atalho_de_volta(order_id):
    """Manda pro PRÓPRIO entregador um push que serve de botão "voltar ao Inksa".

    POR QUE ISTO EXISTE

    O entregador aperta "Dirigir", o Waze assume a tela — e não há volta. O Waze
    NÃO mostra botão de retorno pro app que o chamou (isso só existe pra quem
    tem acordo de SDK com eles), e no Android um app não pode trazer outro pra
    frente. O Diego procurou o caminho de volta em 13/09/2026 e não achou.

    O que sobra é o que o sistema operacional já sabe fazer: notificação. Ela
    fica na barra enquanto o Waze ocupa a tela, e tocar nela abre o app — o
    listener `pushNotificationActionPerformed` do app já leva pra tela da
    corrida. Ou seja: as duas pontas existiam, faltava disparar.

    ⚠️ NÃO é urgente de propósito. Isto é um atalho, não um alarme: som alto
    aqui competiria com a voz do Waze guiando a pessoa numa conversão.

    ⚠️ Sem plugin novo e SEM RELEASE NA LOJA. O app do entregador já tem
    @capacitor/push-notifications e já trata o toque. Fazer por notificação
    local exigiria plugin novo, build nativo e dias de espera na Play Store.

    Falhar aqui não pode atrapalhar: quem chama dispara e segue pro Waze sem
    esperar resposta.
    """
    conn = None
    try:
        ...
        if error:
            return error
        if user_type != 'delivery':
            return jsonify({"error": "Apenas o entregador da corrida"}), 403

        conn = get_db_connection()
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            # O pedido é MESMO dele? Sem isto, qualquer entregador logado
            # mandaria notificação usando o número de um pedido alheio.
            cur.execute("""
                SELECT o.numero, o.status, rp.restaurant_name
                  FROM orders o
                  JOIN delivery_profiles dp ON dp.id = o.delivery_id
                  LEFT JOIN restaurant_profiles rp ON rp.id = o.restaurant_id
                 WHERE o.id = %s AND dp.user_id = %s
            """, (str(order_id), user_auth_id))
            pedido = cur.fetchone()
            if not pedido:
                return jsonify({"error": "Esta corrida não é sua"}), 403

            token = _get_fcm_token(cur, 'delivery_profiles', user_auth_id)

        if not token:
            # Não é erro do chamador: ele fez a parte dele. Só não há como avisar.
            return jsonify({"enviado": False, "motivo": "sem token de push"}), 200

        # O texto diz o PASSO, não "volte ao app": quem está dirigindo precisa
        ...
        logger.warning("atalho-de-volta falhou em %s: %s", order_id, e)
