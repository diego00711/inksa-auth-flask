...
# Status internos (usados na lógica e no banco de dados)
VALID_STATUSES_INTERNAL = {
    'pending', 'accepted', 'preparing', 'ready', 
    'delivering', 'delivered', 'cancelled'
}

# Mapeamento para exibição (tradução para o frontend)
STATUS_DISPLAY_MAP = {
    'pending': 'Pendente',
    'accepted': 'Aceito',
    'preparing': 'Preparando',
    'ready': 'Pronto',
    'delivering': 'Saiu para Entrega', # PADRONIZADO
    'delivered': 'Entregue',
    'cancelled': 'Cancelado'
}
        ...
        if request.method == 'GET':
            ...
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                order_data = {
                    ...
                    'status': 'pending', 'pickup_code': generate_verification_code(), 'delivery_code': generate_verification_code()
                }
...
def update_order_status(order_id):
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            logger.info(f"Status do pedido {order_id} atualizado para {new_status_internal}")
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            sql_query = """
                SELECT o.id, o.restaurant_id, rp.restaurant_name, o.delivery_id as deliveryman_id,
                       (dp.first_name || ' ' || dp.last_name) as deliveryman_name, o.updated_at as completed_at
                FROM orders o
                JOIN restaurant_profiles rp ON o.restaurant_id = rp.id
                LEFT JOIN delivery_profiles dp ON o.delivery_id = dp.id
                WHERE o.client_id = %s AND o.status = 'delivered' AND (
                    NOT EXISTS (SELECT 1 FROM restaurant_reviews rr WHERE rr.order_id = o.id AND rr.client_id = %s) OR 
                    (o.delivery_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM delivery_reviews dr WHERE dr.order_id = o.id AND dr.client_id = %s))
...
@orders_bp.route('/pending-delivery-review', methods=['GET'])
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            sql_query = """
                ...
                WHERE o.delivery_id = %s AND o.status = 'delivered' AND NOT EXISTS (
                    SELECT 1 FROM delivery_reviews dr WHERE dr.order_id = o.id AND dr.delivery_id = %s
                ) ORDER BY o.updated_at DESC;
            """
