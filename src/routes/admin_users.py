...
def update_user(user_id):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute(
                "SELECT id, user_type, email FROM users WHERE id = %s", (str(user_id),)
            )
            user = cur.fetchone()
            ...
            if "user_type" in data:
                ...
                if new_user_type not in valid_types:
                    return (
                        jsonify(
                            {
                                "status": "error",
                                "message": f"Tipo de usuário inválido. Deve ser um de: {', '.join(valid_types)}",
                            }
                        ),
                        400,
                    )
                ...
                update_details.append(
                    f"user_type: {user['user_type']} -> {new_user_type}"
                )
            ...
            if updates:
                ...
                if cur.rowcount == 0:
                    return (
                        jsonify(
                            {"status": "error", "message": "Falha ao atualizar usuário"}
                        ),
                        500,
                    )
