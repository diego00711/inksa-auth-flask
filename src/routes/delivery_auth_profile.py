    ...
    def decorated_function(*args, **kwargs):
        ...
        if isinstance(token_result, tuple) and len(token_result) == 2:
            ...
            if user_type != "entregador":
                return (
                    jsonify(
                        {"error": "Acesso não autorizado. Apenas para entregadores."}
                    ),
                    403,
                )
...
def handle_profile():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute(
                "SELECT * FROM delivery_profiles WHERE user_id = %s", (user_id,)
            )
            ...
            if request.method == "GET":
                ...
                if not request.is_json:
                    return (
                        jsonify({"error": "Content-Type deve ser application/json"}),
                        400,
                    )
                ...
                if not update_data:
                    return (
                        jsonify({"error": "Nenhum campo válido para atualização"}),
                        400,
                    )
...
def upload_avatar():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute(
                "SELECT id FROM delivery_profiles WHERE user_id = %s", (user_id,)
            )
            profile = cur.fetchone()
            if not profile:
