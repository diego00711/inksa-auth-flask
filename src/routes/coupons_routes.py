...
def validate_coupon():
    ...
    try:
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute("""
                -- ⚠️ `*` DE PROPÓSITO. NÃO trocar por lista de colunas.
                --
                -- Aqui havia uma lista enumerada, e ela esqueceu duas colunas
                -- que `evaluate_coupon` lê. Coluna que falta num SELECT não dá
                -- erro nenhum: o `.get()` devolve None e a regra some calada.
                -- Em 13/09/2026, no teste do Diego:
                --   menu_item_id ausente    -> a oferta presa a item caía no
                --      switch de discount_type e virava "Tipo de cupom
                --      inválido", com a reserva do cliente ainda correndo
                --   reserva_minutos ausente -> TODA a trava da relâmpago
                --      (entre na conta / toque na oferta / expirou) inerte
                -- O fechamento do pedido nunca teve o furo — lá é `select('*')`.
                --
                -- A linha NUNCA vai crua pra resposta: o retorno lá embaixo é
                -- montado campo a campo. Então `*` não expõe coluna nova.
                SELECT *
                FROM public.coupons
                WHERE UPPER(code) = %s
...
def cupons_disponiveis():
    ...
    try:
        with conn, conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute("""
                -- ⚠️ `*` de propósito, mesmo motivo do /validate lá em cima.
                -- A saída desta rota também é montada campo a campo.
                SELECT *
                  FROM public.coupons
                 WHERE is_active
