...
@delivery_orders_bp.route('/orders/<uuid:order_id>', methods=['GET'], endpoint='get_single_order_details')
@cross_origin(origins=["http://localhost:5173", "http://127.0.0.1:5173", "http://localhost:3000"], 
              supports_credentials=True, methods=['GET', 'OPTIONS'], 
...
@cross_origin(origins=["http://localhost:5173", "http://127.0.0.1:5173", "http://localhost:3000"], 
              supports_credentials=True, methods=['POST', 'OPTIONS'], 
              allow_headers=['Content-Type', 'Authorization'])
@delivery_token_required
def accept_delivery(order_id):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            cur.execute("SELECT status FROM orders WHERE id = %s AND delivery_id = %s", (str(order_id), profile_id))
            order = cur.fetchone()
            ...
            if order['status'] != 'Pendente': # Ajuste o status conforme seu DB
                return jsonify({"status": "error", "message": f"Pedido já está em status '{order['status']}' e não pode ser aceito."}), 400

            new_status = 'Aceito' # Ajuste o novo status conforme seu DB
            cur.execute("UPDATE orders SET status = %s WHERE id = %s AND delivery_id = %s RETURNING id, status",
                        (new_status, str(order_id), profile_id))
            updated_order = cur.fetchone()
            conn.commit()

            if updated_order:
                return jsonify({"status": "success", "message": f"Pedido {str(order_id)} aceito com sucesso. Novo status: {updated_order['status']}"}), 200
            else:
                conn.rollback()
                return jsonify({"status": "error", "message": "Falha ao aceitar o pedido."}), 500
...
def complete_delivery(order_id):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            cur.execute("SELECT status, delivery_fee FROM orders WHERE id = %s AND delivery_id = %s AND status = ANY(%s)",
                        (str(order_id), profile_id, current_status))
            order = cur.fetchone()

            if not order:
                return jsonify({"status": "error", "message": "Pedido não encontrado, não atribuído a você ou em status inválido para conclusão."}), 404
            ...
            cur.execute("UPDATE delivery_profiles SET total_deliveries = COALESCE(total_deliveries, 0) + 1 WHERE id = %s",
                        (profile_id,))
            
            # --- NOVO: Chamar a rota interna para adicionar pontos ---
            points_for_delivery = 10 # Define quantos pontos a entrega vale. AJUSTE ESTE VALOR.
            ...
            if not points_added_successfully:
                print("AVISO: Falha ao adicionar pontos ao entregador. A transação da entrega será revertida.")
                conn.rollback() 
                return jsonify({"status": "error", "message": "Pedido concluído, mas falha ao atribuir pontos."}), 500

            conn.commit() # Commit a transação AGORA, incluindo a atualização do pedido e dos pontos

            if updated_order:
                return jsonify({"status": "success", "message": f"Pedido {str(order_id)} concluído com sucesso e pontos atribuídos."}), 200
            else:
                conn.rollback()
                return jsonify({"status": "error", "message": "Falha ao atualizar o status do pedido"}), 500
