...
def get_restaurant_details(conn, restaurant_id):
    with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
        cur.execute(
            "SELECT * FROM restaurant_profiles WHERE id = %s", (str(restaurant_id),)
        )
        ...
        cur.execute(
            "SELECT * FROM menu_items WHERE restaurant_id = %s", (str(restaurant_id),)
        )
        ...
        return jsonify(
            {
                "status": "success",
                "data": {**dict(restaurant), "menu_items": menu_items},
            }
        )
...
def handle_profile():
    ...
    try:
        ...
        if request.method == "GET":
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    "SELECT * FROM restaurant_profiles WHERE user_id = %s", (user_id,)
                )
                profile = cur.fetchone()
                if not profile:
                    return (
                        jsonify({"status": "error", "error": "Profile not found"}),
                        404,
                    )
            ...
            if not updates:
                return (
                    jsonify({"status": "error", "error": "No valid fields to update"}),
                    400,
                )
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not updated_row:
                return (
                    jsonify(
                        {"status": "error", "error": "Restaurant profile not found"}
                    ),
                    404,
                )
