...
def get_menu_items(conn):
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
        cur.execute(
            "SELECT id, name, description, price, category, is_available, image_url FROM menu_items WHERE user_id = %s ORDER BY category, name",
            (user_id,),
        )
...
def add_menu_item(conn):
    ...
    if not all(field in data for field in required):
        return (
            jsonify(
                {"status": "error", "error": f"Missing required fields: {required}"}
            ),
            400,
        )
    try:
...
def delete_menu_item(conn, item_id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                "DELETE FROM menu_items WHERE id = %s AND user_id = %s RETURNING id",
                (item_id, user_id),
            )
            ...
            if deleted_id:
                ...
                return (
                    jsonify(
                        {
                            "status": "success",
                            "message": f"Item com ID {item_id} excluído com sucesso.",
                        }
                    ),
                    200,
                )
...
def upload_menu_item_image():
    ...
    if "file" not in request.files:
        return (
            jsonify(
                {
                    "status": "error",
                    "error": "Nenhum ficheiro de imagem enviado com o campo 'file'.",
                }
            ),
            400,
        )
    ...
    if file.filename == "":
        return jsonify({"status": "error", "error": "Nome de ficheiro vazio."}), 400
    try:
        ...
        public_url = supabase.storage.from_("menu-images").get_public_url(
            path_on_storage
        )
