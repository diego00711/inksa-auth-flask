...
def admin_login():
    ...
    if not email or not password:
        return (
            jsonify({"status": "error", "message": "Email e senha são obrigatórios"}),
            400,
        )
        ...
        if not db_user or db_user["user_type"] != "admin":
            ...
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "Acesso não permitido. Apenas para administradores.",
                    }
                ),
                403,
            )
...
def admin_logout():
    ...
    try:
        ...
        return (
            jsonify({"status": "success", "message": "Logout realizado com sucesso"}),
            200,
        )

    except Exception as e:
        return (
            jsonify({"status": "error", "message": f"Erro durante logout: {str(e)}"}),
            500,
        )
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
            ...
            if filter_city:
                where_clauses.append(
                    "COALESCE(cp.address_city, rp.address_city, dp.address_city) ILIKE %s"
                )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Erro interno ao buscar dados do gráfico.",
                    "detail": str(e),
                }
            ),
            500,
...
def update_restaurant(restaurant_id):
    ...
    if not data:
        return (
            jsonify(
                {"status": "error", "message": "Nenhum dado enviado para atualização."}
            ),
            400,
        )
    ...
    try:
        with conn.cursor() as cur:
            ...
            log_admin_action_auto(
                "UpdateRestaurant",
                f"Updated restaurant {restaurant_id} fields: {restaurant_fields}",
            )

        return (
            jsonify(
                {"status": "success", "message": "Restaurante atualizado com sucesso."}
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Erro interno ao atualizar o restaurante.",
                    "detail": str(e),
                }
            ),
            500,
