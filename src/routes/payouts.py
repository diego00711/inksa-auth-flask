...
def handle_process_payouts():
    ...
    if user_type != "admin":
        logging.warning(
            f"Tentativa de acesso não autorizado à rota de processamento de pagamentos pelo usuário {user_auth_id}."
        )
    ...
    if not partner_type or partner_type not in ["restaurant", "delivery"]:
        return (
            jsonify(
                {
                    "error": "Parâmetro 'partner_type' é obrigatório e deve ser 'restaurant' ou 'delivery'."
                }
            ),
            400,
        )
    ...
    if not cycle_type or cycle_type not in ["weekly", "bi-weekly", "monthly"]:
        return (
            jsonify(
                {
                    "error": "Parâmetro 'cycle_type' é obrigatório e deve ser 'weekly', 'bi-weekly' ou 'monthly'."
                }
            ),
            400,
        )
    ...
    logging.info(
        f"Administrador {user_auth_id} iniciou o processamento de pagamentos para '{partner_type}' no ciclo '{cycle_type}'."
    )
    ...
    try:
                ...
                jsonify(
                    {
                        ...
                        "details": generated_payouts["error"],
                    }
                ),
                500,
            )
        ...
        logging.error(
            f"Erro inesperado na rota de processamento de pagamentos: {e}",
            exc_info=True,
        )
