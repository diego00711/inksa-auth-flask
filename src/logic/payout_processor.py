...
def get_date_range_for_cycle(cycle_type):
    ...
    if cycle_type == "weekly":
        ...
        raise ValueError(
            "Tipo de ciclo inválido. Use 'weekly', 'bi-weekly', ou 'monthly'."
        )
...
def process_payouts_for_cycle(cycle_type, partner_type):
    ...
    if not conn:
        logging.error(
            "Não foi possível conectar à base de dados para processar pagamentos."
        )
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"SELECT id FROM {profile_table} WHERE payout_cycle = %s", (cycle_type,)
            )
            ...
            for partner in eligible_partners:
                ...
                logging.info(
                    f"Gerado payout {new_payout_id} de R${total_amount:.2f} para o parceiro {partner_id}."
                )
